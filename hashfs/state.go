	...
	for i, ent := range state.Entries {
		eg.Go(func() error {
			...
			if now := time.Now(); fi.ModTime().After(now) {
				glog.Warningf("future timestamp on %q: mtime=%s now=%s", ent.Name, fi.ModTime(), now)
			...
			case entryAfterLocal:
				...
				glog.Infof("old local %s %q: state:%s disk:%s cmdhash:%s", ftype, ent.Name, e.mtime, fi.ModTime(), base64.StdEncoding.EncodeToString(e.cmdhash))
				if logw != nil {
...
func (hfs *HashFS) journalEntry(ctx context.Context, fname string, e *entry) {
	...
	err := enc.Encode(ent)
	if err != nil {
		glog.Warningf("Failed to marshal journal entry %s: %v", fname, err)
		return
	}
	hfs.journalMu.Lock()
