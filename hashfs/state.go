...
func (hfs *HashFS) SetState(ctx context.Context, state *pb.State) error {
	...
	for i, ent := range state.Entries {
		eg.Go(func() error {
			...
			case entryBeforeLocal:
				...
				le := newLocalEntry()
				le.init(ctx, ent.Name, hfs.executables)
	...
	log.Infof("load state done: eq:%d new:%d not-exist:%d fail:%d invalidate:%d: %s", neq.Load(), nnew.Load(), nnotexist.Load(), nfail.Load(), ninvalidate.Load(), time.Since(start))
	return nil
}
