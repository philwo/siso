...
loop:
	for {
		...
		case err := <-errch:
			...
			if glog.V(1) {
				glog.Infof("errs=%d numServs=%d hasReady=%t stuck=%t", nerrs, numServs, hasReady, stuck)
			}
			if nerrs >= b.failuresAllowed || stuck {
				glog.Infof("unable to proceed nerrs=%d numServs=%d hasReady=%t stuck=%t", nerrs, numServs, hasReady, stuck)
				cancel()
				break loop
			}
			continue
